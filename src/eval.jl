...
@fastmath function (interp::ChebPoly{N})(x::SVector{N,<:Real}) where {N}
    ...
    interp.extrapolate || all(abs.(x0) .≤ 1) || throw(DomainError(x, "not in domain $(interp.lb) to $(interp.ub)"))
...
function chebjacobian(c::ChebPoly{N}, x::SVector{N,<:Real}) where {N}
    ...
    c.extrapolate || all(abs.(x0) .≤ 1) || throw(DomainError(x, "not in domain $(c.lb) to $(c.ub)"))
